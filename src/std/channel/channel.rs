...
use crate::std::channel::context::Context;
use crate::std::channel::counter;
use crate::std::channel::err::{
    RecvError, RecvTimeoutError, SendError, SendTimeoutError, TryRecvError, TrySendError,
};
use crate::std::channel::flavors;
use crate::std::channel::select::{Operation, SelectHandle, Token};
...
/// ```
/// use std::time::Duration;
/// use cogo::std::channel::{after, select, unbounded};
///
...
/// use cogo::std::channel::{at, select, unbounded};
///
/// let (s, r) = unbounded::<i32>();
...
/// use std::thread;
/// use std::time::Duration;
/// use cogo::std::channel::{after, select, never, unbounded};
///
/// let (s, r) = unbounded();
///
/// thread::spawn(move || {
...
impl<T> Sender<T> {
    ...
    ///
    /// ```
    /// use cogo::std::channel::{bounded, TrySendError};
    ///
    /// let (s, r) = bounded(1);
    ///
    ...
    /// use std::thread;
    /// use std::time::Duration;
    /// use cogo::std::channel::{bounded, SendError};
    ///
    /// let (s, r) = bounded(1);
    ...
    /// use std::thread;
    /// use std::time::Duration;
    /// use cogo::std::channel::{bounded, SendTimeoutError};
    ///
    /// let (s, r) = bounded(0);
    ///
    /// thread::spawn(move || {
    ...
    ///
    /// ```
    /// use cogo::std::channel::{bounded, unbounded};
    ///
...
impl<T> Receiver<T> {
    ...
    ///
    /// ```
    /// use cogo::std::channel::{unbounded, TryRecvError};
    ///
    /// let (s, r) = unbounded();
    ...
    /// use std::thread;
    /// use std::time::Duration;
    /// use cogo::std::channel::{unbounded, RecvTimeoutError};
    ///
    /// let (s, r) = unbounded();
    ///
    /// thread::spawn(move || {
